var Asset = function($, e) {
	...
	this.id = parseInt(url.match(/=(\d+)/)[1], 10);
	...
	if (isLimitedUnique) {
		...
		this.serialNumber = parseInt(str[1], 10);
		this.totalSold = parseInt(str[2], 10);
	}
...
var Profile = function($) {
	...
	if (label.length > 0) {
		...
		this.currentPage = parseInt(matches[1], 10);
		this.totalPages = parseInt(matches[2], 10);
	}
	var assets = [];
